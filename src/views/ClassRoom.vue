      ...
      <div class="flex-row dashboard _1">
        ...
        <div class="flexcolumn classroom">
              ...
              <div class="lessonblock" v-for="(lesson, index) in currentClass.lessons" :key="lesson._id" :class="{active: index === currentLessonIndex, white: index !== currentLessonIndex}" @click="currentLessonIndex = index">
                <div class="div-block-9">
                  <div class="div-block-7 classroom">
                    ...
                    <h1 class="heading-4" :class="{white: index === currentLessonIndex}">
                      {{lesson.title.length <= 33 ?lesson.title : lesson.title.slice(30)+ '...'}}</h1>
                  </div>
                </div>
                <div class="div-block-8">
                  <div class="text-block" :class="{white: index === currentLessonIndex}">{{lesson.secondsDuration | duration }}</div>
                  <div v-if="isInstructor && lesson.studentAssignments.length" class="notification">{{lesson.studentAssignments.length}}</div>
                  <div v-if="lesson.teacherAssignment && !isInstructor">
          ...
          <div class="card">
            ...
            <div class="div-block-126" v-if="currentLesson.teacherAssignment && !isInstructor">
              <a href="#" @click.prevent="openSubmitAssignment" class="link">Submit Assignment</a>
            </div>
            <div class="div-block-126" v-if="isInstructor">
              <a href="#" @click.prevent="openEditClass()" class="link">Edit</a>
            </div>
          </div>
          <div class="div-block-145" v-if="newAssignments && newAssignments.length">
            <h1 class="heading-6 grey">NEW</h1>
          </div>
          <project-card v-for="assign in newAssignments" isAssignment="true" :key="assign._id" :project="assign" />
          <div class="div-block-145" v-if="oldAssignments && oldAssignments.length">
            <h1 class="heading-6 grey">Earlier</h1>
          </div>
          <project-card v-for="assign in oldAssignments" isAssignment="true" :key="assign._id" :project="assign" />
...
<script>
  ...
  import ProjectCard from "../components/Feeds/ProjectCard";
  export default {
    name: "ClassRoom",
    props: ["id"],
    components: {
    ...
    computed: {
      ...
      isInstructor() {
        return this.loggedInUser._id === this.currentClass.instructor._id;
      },
      newAssignments() {
        return this.currentLesson && this.currentLesson.studentAssignments ? this.currentLesson.studentAssignments.filter(sAssign => sAssign.status === 'in-review') : [];
      },
      oldAssignments() {
        return this.currentLesson && this.currentLesson.studentAssignments ? this.currentLesson.studentAssignments.filter(sAssign => sAssign.status != 'in-review') : [];
      }
    ...
    methods: {
      openEditClass() {
        this.$store.dispatch("classes/setEditClass", {
          ...
          currentClass: JSON.parse(JSON.stringify(this.currentClass))
        });
