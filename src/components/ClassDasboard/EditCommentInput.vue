      ...
      <div class="form-block-3 w-form">
        <form id="email-form" name="email-form" data-name="Email Form">
          ...
          <div class="comment-block w-input pd-0 pdr-10">
            <img src="@/assets/attachment.svg" v-if="!selectedMedia.file" @click="chooseMedia()" class="attach" />
  ...
  export default {
    ...
    methods: {
      ...
      addMedia(fieldName, fileList) {
        ...
        this.selectedMedia.file = newMedia;
        this.selectedMedia.mediaType = newMedia.type.split("/")[0];
      ...
      removeMedia() {
        if (!this.replaceOldMedia && this.discItem.media && this.discItem.media.mediaId) {
          this.replaceOldMedia = true
        }
        this.selectedMedia.mediaType = "";
        this.selectedMedia.file = null;
      ...
      submitComment() {
        if (this.discItem.body) {
          ...
          // delete this.discItem.media;
             this.$set(this.discItem, 'media', null);
            this.onSubmit();
          }
        }
      },
      uploadMedia() {
        ...
        formData.append("prodeusFiles", this.selectedMedia.file);
        this.isUploading = true;
        this.$store.dispatch("classes/uploadFiles", formData).then(
          media => {
            this.isUploading = false;
            console.log(this.selectedMedia.mediaType);
            if(!this.discItem.media) {
              // let discItemCopy = Object.assign({}, this.discItem);
              let discItemCopy = { mediaId: "", type: this.selectedMedia.mediaType}
             this.$set(this.discItem, 'media', discItemCopy);
            } else {
              this.discItem.media.type = this.selectedMedia.mediaType
            }
            this.discItem.media.mediaId = media.data[0];
            this.replaceOldMedia = false;
            this.removeMedia();
            this.onSubmit();
          },
          err => {
    ...
    computed: {
      ...
      isVideo() {
        return (this.selectedMedia.mediaType === "video") ||
        (this.isAlreadyMedia && this.discItem.media.type === 'video');
      },
      isMedia() {
        return (this.isAlreadyMedia) || !!this.selectedMedia.file;
      },
      isAlreadyMedia(){
        return this.discItem.media && this.discItem.media.mediaId && !this.replaceOldMedia
      },
      src() {
